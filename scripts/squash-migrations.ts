...
execSync(
  `pnpx prisma migrate diff --from-empty --to-schema-datamodel ./prisma/schema.prisma --script > ${migrationPath}/migration.sql`,
  { stdio: 'inherit' },
);

if (apply) {
  console.log('> Apply new migration...');
  execSync(`pnpx prisma migrate resolve --applied ${migrationName}`, {
    stdio: 'inherit',
  });
}
